...
//	tablaHtml+="<th hidden='hidden'>&nbsp;</th>";
	...
	oTable =$('#datatableActividades').dataTable({
        ...
        "aoColumns": [
						{ "mDataProp": "codigoActividad", "bSortable": false, "sWidth":"30%" },
						{ "mDataProp": "nombreActividad","bSortable": false, "sWidth":"30%" },
		  ...
		  success: function (res) {
			  if(res.cdactividad != null){
				...
				$.each(res.tareaActividadList, function(index, optionData) {
					...
					var div = document.getElementById('papeleraTarea');
					var nuevasActividades = div.parentNode.parentNode;	
	     ...
	     type : "POST",
		  ...
		  success: function (res) {
			  if(res.STATUS =="SUCCESS"){
				  $("#messageErrorModal").html("No se puede guardar la actividad, ya existe una actividad con el mismo nombre.")
	     ...
	     type : "POST",
		  ...
		  success: function (res) {
			  if(res.STATUS =="SUCCESS"){
				  ...
				  $("#messageError").html("No se puede eliminar la actividad, ya se encuentra asociada a uno o varios tipos de casos.")
				  $("#messageError").show(); 
	...
	if ( $.fn.dataTable.isDataTable( '#tablaTareas' ) ) {
		...
		.fnAddData(
				[
						'' + $("#nombreTarea").val(),
						'' + $("#detalleTarea").val(),
						'<a href="javascript:void(0);" onclick="deleteTarea('+filaTarea+')"  title="Eliminar Tarea"><i class="fa fa-bitbucket" style="color: red;"></i></a></a>'
						+'<a href="javascript:void(0);" onclick="editTarea('+filaTarea+')" title="Editar  Tarea" ><i class="glyphicon glyphicon-edit"></i></a>']);
	...
	for ( var i = 0; i < tareasArray.length; i++) {
		if (tareasArray[i].fila==fila) {
			$("#f" + fila + " td.nombreTarea").html(
					'<input id="codigoTareaActividadEditado'+fila+'" name="codigoTareaActividadEditado' + fila+'" type="hidden" class="form-control" required="required" value="'
					+ tareasArray[i].codigoTarea + '"/>  '
					+'<input id="nombreTareaEditado'+fila+'" name="nombreTareaEditado' + fila+'" type="text" class="form-control" required="required" value="'
					+ tareasArray[i].nombreTarea + '"/>  ');
			$("#f" + fila + " td.detalleTarea").html(
					'<input id="detalleTareaEditado'+fila+'" name="detalleTareaEditado' + fila+'" type="text" class="form-control" required="required" value="'
					+ tareasArray[i].detalleTarea + '"/>  ');
					
			$("#f" + fila + " td.accionTarea")
			.html(
					'<a href="javascript:void(0);" onclick="cambiarTarea('+fila+','+i+')" class="btn btn-success btn-circle"><i class="glyphicon glyphicon-ok"></i></a>');
		}
	}
...
function cambiarTarea(fila, posicion) {
	...
	tareasArray[posicion] = elementoTarea;
		...
		$("#f" + fila + " td.accionTarea")
				.html(
						'<a href="javascript:void(0);" onclick="deleteTarea('+fila+')" title="Eliminar Tarea"><i class="fa fa-bitbucket" style="color: red;"></i></a></a>'
						+'<a href="javascript:void(0);" onclick="editTarea('+fila+')" title="Editar Tarea"><i class="glyphicon glyphicon-edit"></i></a>');
...
function eliminarActividad(codigo){
	/*if(casoActividad==true){
		$("#modal-NoEliminarActividad").modal("show");
